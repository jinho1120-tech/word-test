...
export function WordManager({
  ...
  function handleClearAll() {
    if (!window.confirm("오늘 추가한 모든 단어를 정말로 삭제하시겠습니까?")) {
      return
    }
    startTransition(async () => {
              ...
              <li key={w.id} className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3 overflow-hidden">
                {editingId === w.id ? (
                  <div className="flex w-full flex-col gap-2 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <input value={editExample} onChange={(e) => setEditExample(e.target.value)} className="w-full rounded-lg border border-input px-3 py-1.5 text-sm outline-none focus:border-primary" placeholder="영어 뜻/힌트" />
