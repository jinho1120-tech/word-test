  ...
  if (words.length === 0) {
    return (
      <div className="rounded-3xl border border-dashed border-border bg-muted/30 px-6 py-12 text-center">
        <p className="text-base font-semibold text-foreground">아직 오늘의 단어가 없어요</p>
    ...
    <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
      {phase === "start" && (
        <div className="flex flex-col items-center px-6 py-12 text-center">
          <div
            className="mb-5 flex size-16 items-center justify-center rounded-2xl text-white"
          ...
          <button
            onClick={() => begin(words)}
            className="w-full rounded-2xl py-4 text-lg font-bold text-white shadow-md transition-opacity hover:opacity-90 active:opacity-80"
            style={{ backgroundColor: accent }}
          >
            퀴즈 시작하기
          </button>
        ...
        <>
          ...
          <div className="flex flex-col px-6 py-8">
            <div className="mb-6 flex items-center justify-between text-xs font-medium text-muted-foreground">
              <span>
                점수 <span className="font-bold text-foreground">{score}</span>
              </span>
              <span className="rounded-full bg-muted px-3 py-1 font-semibold text-foreground">
                {index + 1} / {total}
              </span>
              <span>
                연속 <span className="font-bold" style={{ color: accent }}>{streak}</span>
              </span>
            </div>
            ...
            <input
              ...
              className={cn(
                "mb-3 w-full border-b-4 bg-transparent p-3 text-center text-3xl font-bold outline-none transition-colors placeholder:text-base placeholder:font-normal placeholder:text-muted-foreground",
                feedback === "idle" && "border-border text-foreground",
                feedback === "correct" && "border-green-500 text-green-600",
                feedback === "wrong" && "animate-shake border-red-500 text-red-500",
              )}
            ...
            <div className="mb-6 flex min-h-6 items-center justify-center">
              ...
              {feedback === "wrong" && (
                <p className="flex items-center gap-1.5 text-sm font-semibold text-red-500">
                  <X className="size-4" /> 정답: {current.word}
                </p>
              )}
              {feedback === "idle" && hintUsed && (
                <p className="text-sm text-muted-foreground">
                  첫 글자: <span className="font-bold text-foreground">{current.word[0]}</span>
                </p>
              )}
            </div>
            ...
            >
              {feedback === "idle" && (
                <>
                  정답 확인 <ArrowRight className="size-5" />
                </>
              )}
            ...
            <button
              ...
              className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-2xl border border-border py-3 text-sm font-semibold text-muted-foreground transition-colors hover:bg-muted disabled:opacity-40"
            >
              <Lightbulb className="size-4" /> 힌트 보기
            </button>
          </div>
        </>
      )}
      ...
      {phase === "result" && (
        <div className="flex flex-col px-6 py-10">
          <div className="mb-6 flex flex-col items-center text-center">
            <div
              className="mb-3 flex size-16 items-center justify-center rounded-2xl text-white"
              style={{ backgroundColor: accent }}
            >
          ...
          <div className="mb-6 rounded-2xl bg-muted/50 p-6 text-center">
            ...
            <div className="mt-4 flex justify-center gap-6 text-sm text-muted-foreground">
              <span>
                정답 <span className="font-bold text-foreground">{correctCount}</span>/{total}
              </span>
              <span>
                최고 연속 <span className="font-bold text-foreground">{bestStreak}</span>
              </span>
            </div>
          </div>
